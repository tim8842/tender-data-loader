...
func TestGetParamFromHref(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := GetParamFromHref(tt.href, tt.param)
...
func TestParseAgreementIds(t *testing.T) {
	...
	dir := "test_docs/ParseAgreementIds"
	...
	tests := []struct {
		...
		{
			name:      "bad doc",
			inputHTML: baseutils.ReadHtmlFile(dir + "/error.html"),
			expectErr: false,
			expected:  []string{},
		},
		{
			name:      "correct doc",
			inputHTML: baseutils.ReadHtmlFile(dir + "/correct.html"),
			expectErr: false,
			expected: []string{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result, err := ParseAgreementIds(ctx, logger, tt.inputHTML)
			if tt.expectErr {
				assert.Error(t, err)
			} else {
...
func TestParseAgreementFromMain(t *testing.T) {
	ctx := context.Background()
	logger, _ := zap.NewDevelopment()
	dir := "test_docs/ParseAgreementFromMain"
	tests := []struct {
		name         string
		inputHTML    []byte
		expectErr    bool
	...
	}{
		{
			...
			expectedData: &model.AgreementParesedData{
				...
				UpdatedAt: getDate("11.06.2025"), Customer: &model.Customer{ID: "492275"},
			},
		},
		{
			name:         "error",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result, err := ParseAgreementFromMain(ctx, logger, tt.inputHTML)
			if tt.expectErr {
				assert.Error(t, err)
			} else {
				assert.NoError(t, err)
...
func TestParseAgreementFromHtml(t *testing.T) {
	ctx := context.Background()
	logger, _ := zap.NewDevelopment()
	dir := "test_docs/ParseAgreementFromHtml"
	tests := []struct {
		name         string
		inputHTML    []byte
		inputData    *model.AgreementParesedData
		expectErr    bool
		expectedData *model.AgreementParesedData
	}{
		{
			name:      "correctNew2024",
			inputHTML: baseutils.ReadHtmlFile(dir + "/correctNew2024.html"),
			inputData: &model.AgreementParesedData{
				ID: "", Number: "56680005928250002570000",
				Status: "Исполнение завершено", Pfid: "67051964",
			...
			expectedData: &model.AgreementParesedData{
				...
				SignedAt: getDate("23.05.2025"), ExecutionStart: getDate("23.05.2025"),
				ExecutionEnd: getDate("19.06.2025"), PublishedAt: getDate("23.05.2025"),
				UpdatedAt: getDate("11.06.2025"), Customer: &model.Customer{
				...
				Services: []*model.AgreementService{
					{
		...
		{
			name:      "correctOld2013",
			inputHTML: baseutils.ReadHtmlFile(dir + "/correctOld2013.html"),
			inputData: &model.AgreementParesedData{
				...
				UpdatedAt: getDate("02.03.2015"), Customer: &model.Customer{ID: "87730"},
			},
			expectErr: false,
			expectedData: &model.AgreementParesedData{
				ID: "", Number: "55110001373150000040000",
				...
				SignedAt: getDate("10.06.2013"), ExecutionStart: getDate("10.06.2013"),
				ExecutionEnd: getDate("31.01.2016"), PublishedAt: getDate("16.01.2015"),
				UpdatedAt: getDate("02.03.2015"), Customer: &model.Customer{
					ID: "87730", Code: "",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result, err := ParseAgreementFromHtml(ctx, logger, tt.inputHTML, tt.inputData)
			if tt.expectErr {
				assert.Error(t, err)
			} else {
				assert.NoError(t, err)
...
func TestParseCustomerFromMain(t *testing.T) {
	ctx := context.Background()
	logger, _ := zap.NewDevelopment()
	dir := "test_docs/ParseCustomerFromMain"
	tests := []struct {
		name         string
		inputHTML    []byte
		inputData    *model.AgreementParesedData
	...
	}{
		{
			...
			inputHTML: baseutils.ReadHtmlFile(dir + "/correctTwoMainWork.html"),
			inputData: &model.AgreementParesedData{
		...
		},
		{
			...
			inputHTML: baseutils.ReadHtmlFile(dir + "/corrcorrectOneMainWorkect.html"),
			inputData: &model.AgreementParesedData{
				Customer: &model.Customer{},
			},
			expectErr: false,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := ParseCustomerFromMain(ctx, logger, tt.inputHTML, tt.inputData)
			if tt.expectErr {
				assert.Error(t, err)
			} else {
				assert.NoError(t, err)
